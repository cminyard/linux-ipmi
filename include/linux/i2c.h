...
/* Like above, but don't claim any locks.  This is for mux use only. */
 ...
 * @parms: Parameters passed in from the sysfs add operation.
 ...
 * @master_start: Start the I2C transfer and return immediately.  The
 *   callback in the entry will be called when the operation completes.
 * @smbus_start: Start the smbus transfer and return immediately.  The
 *   callback in the entry will be called when the operation completes.
 * @poll: Poll the adapter for handling.  This is to be called during
 *   a panic when you need to talk to something on the I2C bus but can't
 *   schedule.  It only works with master_start or smbus_start on an
 *   adapter that supports event-driven handling.
